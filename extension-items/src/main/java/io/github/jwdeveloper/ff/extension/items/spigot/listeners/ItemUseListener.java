...
import io.github.jwdeveloper.ff.extension.items.impl.events.FluentItemUseEvent;
...
public class ItemUseListener extends EventBase {
    ...
    public void onEntityRightClick(PlayerInteractAtEntityEvent event) {
        ...
        var optional = handleItemUse(event.getPlayer(), event.getPlayer().getItemInUse(), action);
        if (optional.isEmpty()) {
            return;
        }
    ...
    public void onItemUse(PlayerInteractEvent event) {
        var optional = handleItemUse(event.getPlayer(), event.getItem(), getUseAction(event));
        if (optional.isEmpty()) {
            return;
        }
        Bukkit.getPluginManager().callEvent(optional.get());
    ...
    private Optional<FluentItemUseEvent> handleItemUse(Player player, ItemStack itemStack, FluentItemUseEvent.Action action) {
        if (itemStack == null) {
        ...
        var fluentItemEvent = new FluentItemUseEvent(player, fluentItemStack, action);
        if (action.isShift()) {
