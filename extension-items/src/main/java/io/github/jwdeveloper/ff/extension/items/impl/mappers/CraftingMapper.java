...
public class CraftingMapper implements FluentCraftingMapper {
    ...
    public CraftingMapper(FluentItemRegistry fluentItemRegistry, FluentItemApiSettings settings) {
    ...
    private ActionResult handleSingleSlot(Integer index, String itemName, FluentCrafting fluentCrafting) {
        if (itemName.trim().contains("@")) {
            ...
            fluentCrafting.getSlotsValidators().put(index, itemStack ->
            {
                ...
                if (!container.has(settings.getItemTagKey(), PersistentDataType.STRING)) {
                    return false;
                }
                var value = container.get(settings.getItemTagKey(), PersistentDataType.STRING);
